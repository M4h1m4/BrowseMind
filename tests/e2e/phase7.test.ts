 ...
 *
 * Tests the stuck detection and human handoff flow on OrangeHRM.
...
const TARGET_APP = 'https://opensource-demo.orangehrmlive.com'
...
describe('Phase 7 — Human Escalation E2E', () => {
  itE2E('sets status to stuck when element cannot be found, then fails gracefully on cancelled handoff', async () => {
    ...
    // Cancel handoff after 5 seconds to avoid hanging
    const timer = setTimeout(() => cancelHandoff(state.runId), 5000)
  ...
  itE2E('recovers after human signals handoff done', async () => {
    ...
    // Signal "done" after 5 seconds (simulating human completing the step manually)
    const timer = setTimeout(() => signalHandoff(state.runId, 'I navigated manually'), 5000)
    ...
    // After handoff, step is retried — may succeed or fail depending on page state
    // The key assertion is the stuck log entry with humanNotes
